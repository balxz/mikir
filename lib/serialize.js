...
exports.clientsConfig = async (opsi, store) => {
  ...
  clients.ev.on("group-participants.update", async ({
    id
  }) => {
...
exports.smsg = async (clients, m, store) => {
  ...
  if (m.key) {
    ...
    ? bail.jidNormalizedUser(m.key?.participant || m.participant): bail.jidNormalizedUser(m.key.remoteJid);
  ...
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      if (m.quoted && m.quoted.key) {
        ...
        ? m.quoted.key.participant: m.quoted.key.remoteJid;
  ...
  m.reply = async (text, options = {}) => {
    ...
    //     }
    // );
    return clients.relayMessage(
      m.chat,
      {
        interactiveMessage: {
          header: {
            title: "Shiina Team | SHくƷ",
